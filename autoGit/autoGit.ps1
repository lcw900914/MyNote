...
$labelTaskName.Location = New-Object System.Drawing.Point(30,70)
$labelTaskName.Size = New-Object System.Drawing.Size(120,25)
$form.Controls.Add($labelTaskName)

$textTaskName = New-Object System.Windows.Forms.TextBox
$textTaskName.Location = New-Object System.Drawing.Point(160,68)
$textTaskName.Size = New-Object System.Drawing.Size(330,25)
$form.Controls.Add($textTaskName)
...
$btnCreate.Add_Click({
    ...
    $TaskName   = $textTaskName.Text.Trim()
    ...
    # 基本檢查
    if (-not $folderPath -or -not $userName -or -not $TaskName -or -not $repoURL) {
        $labelStatus.Text = "Please fill in Username, TaskName, and Repo URL, and folderPath."
        return
    }
    ...
    if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
		...
		git init
		git config --global user.email "lspss97175t@gmail.com"
		git config --global user.name "lcw900914"
...
'@
        ...
        #    雙引號要用 反引號 ` 來轉義
        $innerCommand = "`"$psExePath`" -ExecutionPolicy Bypass -File `"$scriptPath`" -RepoPath `"$repoPath`" -UserName `"$userName`" -RepoURL `"$repoUrl`""
